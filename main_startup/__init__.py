...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - [🦊 Fox X Bot] - %(levelname)s - %(message)s",
)
...
if not Config.STRINGSESSION:
    logging.error("No String Session Found! 🦊 Fox X Bot is Exiting!")
    quit(1)

if not Config.API_ID:
    logging.error("No Api-ID Found! 🦊 Fox X Bot is Exiting!")
    quit(1)

if not Config.API_HASH:
    logging.error("No ApiHash Found! 🦊 Fox X Bot is Exiting!")
    quit(1)

if not Config.LOG_GRP:
    logging.error("No Log Group ID Found! 🦊 Fox X Bot is Exiting!")
    quit(1)
